...
public class TestApp implements Application, ScribeClient {
	...
	{
		TestScribeContent tsc = new TestScribeContent(endpoint.getId(), this.toString(), node.getEnvironment().getTimeSource().currentTimeMillis());
	...
	public boolean anycast(Topic topic, ScribeContent content) 
	{
		...
		{
			System.out.println(this + " has passed the anycast onto others");
		}
		return returnValue;
	}
	...
	{
		Message msg = new TestMsg(endpoint.getId(), id, this.toString(), node.getEnvironment().getTimeSource().currentTimeMillis());
	...
	{	
		Message msg = new TestMsg(endpoint.getId(), nh.getId(), this.toString(), node.getEnvironment().getTimeSource().currentTimeMillis());
	...
	public void routeMyMsgLater(NodeHandle nh, int delay)
	{
	 ...
	 {
		 long curr_time = node.getEnvironment().getTimeSource().currentTimeMillis();
		 long sent_time = ((TestMsg)message).time;
		 System.out.println(this + " received a message from " + ((TestMsg)message).owner + ". The message took " + (curr_time - sent_time) + " ms to arrive.");
	 }
